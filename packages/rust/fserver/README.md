...
fserver/
├── src/
│   └── main.rs     # Server entry point and WebSocket handlers
...
The Makefile in the project root handles server startup automatically for DKG and signing flows:

```bash
# Run DKG ceremony (starts fserver automatically)
make dkg out=run_dkg t=2 n=3 gid=mygroup bind=127.0.0.1:9034 KEY_TYPE=secp256k1

# Run signing ceremony over WebSocket (starts fserver automatically)
make ws-sign out=run_dkg t=2 n=3 gid=mygroup bind=127.0.0.1:9034 msg="hello" KEY_TYPE=secp256k1
```
...
A TypeScript implementation of the server is available at `packages/ts/ts-fserver`. It provides the same WebSocket API and can be run with:

```bash
# From project root
npm run dev:server

# Or from ts-fserver directory
npm run dev
```
