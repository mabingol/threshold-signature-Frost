...
#### `group_vk1_to_uncompressed(group_vk_sec1_hex: string)`
Converts a FROST group verifying key (compressed SEC1 format) to its uncompressed point representation.

- **Inputs:**
  - `group_vk_sec1_hex`: `string` - The compressed group verifying key, hex-encoded in SEC1 format.
- **Output:** `string` - The uncompressed public key point, hex-encoded (65 bytes: 0x04 prefix + 32-byte X + 32-byte Y).
...
### Server/Coordinator Aggregation Logic

These functions are intended for the coordinator/server side of a FROST signing session.

#### `compute_signing_package(commitments_map_js: JsValue, msg32_hex: string)`
Aggregates all participants' Round 1 commitments and the message hash into a signing package.

- **Inputs:**
  - `commitments_map_js`: `JsValue` - A `Map<string, string>` of `identifier_hex` (32-byte hex) to `commitments_hex` (bincode-serialized Round 1 commitments).
  - `msg32_hex`: `string` - The 32-byte message hash to be signed, hex-encoded.
- **Output:** `string` - The signing package, bincode-serialized and hex-encoded. This should be broadcast to all signing participants.

#### `aggregate_signatures(signing_package_hex: string, signature_shares_map_js: JsValue, verifying_shares_map_js: JsValue, group_vk_hex: string)`
Aggregates all participants' signature shares into a final threshold signature.

- **Inputs:**
  - `signing_package_hex`: `string` - The signing package from `compute_signing_package`.
  - `signature_shares_map_js`: `JsValue` - A `Map<string, string>` of `identifier_bincode_hex` to `signature_share_bincode_hex`.
  - `verifying_shares_map_js`: `JsValue` - A `Map<string, string>` of `identifier_bincode_hex` to `verifying_share_bincode_hex`.
  - `group_vk_hex`: `string` - The group's verifying key in SEC1 format, hex-encoded.
- **Output:** `string` - A JSON string containing:
  - `signature_bincode_hex`: The final aggregated FROST signature, bincode-serialized.
  - `px`, `py`: The X and Y coordinates of the group public key.
  - `rx`, `ry`: The X and Y coordinates of the signature's R point.
  - `s`: The signature's scalar component.

---

### Verification

#### `verify_signature(roster_pub_json: JsValue, payload_hex: string, signature_hex: string)`
Verifies a signature against a participant's roster public key.

- **Inputs:**
  - `roster_pub_json`: `JsValue` - The participant's roster public key object containing the key type and public key bytes.
  - `payload_hex`: `string` - The original payload that was signed, hex-encoded.
  - `signature_hex`: `string` - The signature to verify, hex-encoded.
- **Output:** `boolean` - Returns `true` if the signature is valid, otherwise throws an error.
